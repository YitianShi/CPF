...
class SimpleTransformImg:
    ...
    def __init__(self, cfg: CN) -> None:
        ...
        self._img_mean = cfg.DATA_PRESET.get("IMG_MEAN", [0.5, 0.5, 0.5])
        self._img_std = cfg.DATA_PRESET.get("IMG_STD", [1, 1, 1])
    ...
    def __call__(self, image, label):
        if self._aug:
            ...
            s_factor = 1 + np.clip(np.random.normal(loc=0, scale=sf), -3 * sf, 3 * sf)
        ...
        affine_2x3 = affine[:2, :]
        patch_width, patch_height = int(self._output_size[0]), int(self._output_size[1])
        image = cv2.warpAffine(image,
                               affine_2x3, (patch_width, patch_height),
        ...
        image = tvF.to_tensor(image)  # (C, H, W)
        image = tvF.normalize(image, self._img_mean, self._img_std)
...
@TRANSFORM.register_module()
class SimpleTransform2D(SimpleTransformImg):
    ...
    def __call__(self, image, label):
        ...
        results["target_joints_2d"] = target_joints_2d

        if self._with_mask:
            patch_width, patch_height = int(self._output_size[0]), int(self._output_size[1])
            mask = cv2.warpAffine(label["image_mask"],
                                  affine[:2, :], (patch_width, patch_height),
                                  flags=cv2.INTER_LINEAR,
                                  borderMode=cv2.BORDER_CONSTANT)
        ...
        if self._with_heatmap:
            n_joints = target_joints_2d.shape[0]  # number of joints
            """ Generate GT Gussian heatmap for joints_2d"""
            heatmap = np.zeros((n_joints, self._heatmap_size[1], self._heatmap_size[0]), dtype="float32")
            imsize = np.array(self._output_size)  # shape (2)
            hmsize = np.array(self._heatmap_size)  # shape (2)
            for i in range(n_joints):
                joint_i = ((target_joints_2d[i] / imsize) * hmsize).astype(np.int32)
                heatmap[i], _ = generate_heatmap(heatmap[i], joint_i, self._heatmap_sigma)

            results["target_joints_heatmap"] = heatmap
            if self._with_mask and self._mask_scale_to_heatmap:
...
class SimpleTransform3DMANO(SimpleTransform3D):
    ...
    def __call__(self, image, label):
        ...
        rot = results["rot_rad"]
        rot_mat = results["rot_mat3d"]
        target_mano_pose = rotate_smpl_pose(label["mano_pose"], rot).reshape(-1, 3)
        target_verts_3d = rot_mat.dot(label["verts_3d"].transpose(1, 0)).transpose()
...
@TRANSFORM.register_module()
class SimpleTransform3DMANOObject(SimpleTransform3DMANO):
    ...
    def __call__(self, image, label):
        results = super().__call__(image, label)
        rotmat = results["rot_mat3d"]

        obj_trasnf = label["obj_transf"]
        obj_rot, obj_tsl = obj_trasnf[:3, :3], obj_trasnf[:3, 3]  # (3, 3), (3, )
        new_obj_rot = rotmat @ obj_rot
        new_obj_tsl = rotmat.dot(obj_tsl)
        target_obj_transf = build_SE3_from_Rt(new_obj_rot, new_obj_tsl)

        obj_verts_3d = label["obj_verts_3d"]
        obj_normals_3d = label["obj_normals_3d"]
        target_obj_verts_3d = rotmat.dot(obj_verts_3d.transpose(1, 0)).transpose(1, 0)
        target_obj_normals_3d = rotmat.dot(obj_normals_3d.transpose(1, 0)).transpose(1, 0)

        results["target_obj_transf"] = target_obj_transf
        results["target_obj_verts_3d"] = target_obj_verts_3d
        results["target_obj_normals_3d"] = target_obj_normals_3d
...
def batch_nviews_cam_extr_transf(batch_cam_extr, batch_joints):
...
def batch_nviews_cam_intr_projection(batch_cam_intr, batch_joints, eps=1e-7):
...
def batch_SE3_transform(verts: torch.Tensor, SE3: torch.Tensor):
    """Batch apply SE3 transform on points

    Args:
        verts (torch.Tensor): 3D points with shape (B, N, 3)
        SE3 (torch.Tensor): SE3 transform matrix with shape (B, 4, 4)

    Returns:
        torch.Tensor: 3D points with shape (B, N, 3)
    """
    R = SE3[:, :3, :3]
    t = SE3[:, :3, 3]
    return batch_Rt_transform(verts, R, t)


def batch_Rt_transform(verts: torch.Tensor, R: torch.Tensor, t: torch.Tensor):
    """Batch apply R and t transform on points

    Args:
        verts (torch.Tensor): 3D points with shape (B, N, 3)
        R (torch.Tensor): SO3 rotation matrix with shape (B, 3, 3)
        t (torch.Tensor): R^3 translation vector with shape (B, 3)

    Returns:
        torch.Tensor: 3D points with shape (B, N, 3)
    """
    return R.bmm(verts.transpose(1, 2)).transpose(1, 2) + t.unsqueeze(1)
...
def persp_project(points3d, cam_intr):
    ...
    """
    hom_2d = cam_intr.dot(points3d.transpose()).transpose()
...
def build_SE3_from_Rt(rot: np.ndarray, transl: np.ndarray):
    """Build SE3 transform matrix from rotation matrix and translation matrix.

    Args:
        rot (np.ndarray): The rotation matrix. (3, 3)
        transl (np.ndarray): The translation matrix. (3, )

    Returns:
        np.ndarray: The SE3 transform matrix. (4, 4)
    """
    SE3 = np.eye(4)
    SE3[:3, :3] = rot
    SE3[:3, 3] = transl
...
def gen_random_rotation(max_angle):
    axisang = gen_random_direction()
    angle = torch.rand(1) * max_angle
    axisang = axisang * angle
    rot_mat = aa_to_rotmat(axisang.unsqueeze(0)).view(1, 3, 3)
    return rot_mat
